...
<head>
  <meta charset="UTF-8">
  <title>FlowBank – Admin Dashboard</title>
...
<header>
  ...
  <div class="nav-buttons">
    <a href="admin-dashboard" class="nav-button">Home</a>
    <a href="admin-logout" class="nav-button">Log Out</a>
  </div>
</header>
<main>
  <%
  ...
  <div class="section">
    ...
    <table>
      ...
      <tr>
        ...
        <td><%= acc.isBlocked() ? "Yes" : "No" %></td>
        <td><%= acc.getBalance() %> ₴</td>
        <td>
          <form method="post" action="admin-accounts">
            <input type="hidden" name="accountId" value="<%= acc.getId() %>">
            <input type="hidden" name="action" value="<%= acc.isBlocked() ? "unblock" : "block" %>">
            <button type="submit"><%= acc.isBlocked() ? "Unblock" : "Block" %></button>
          </form>
        </td>
      </tr>
      <%
      ...
      %>
      <tr>
        <td colspan="6">Немає рахунків для відображення.</td>
      </tr>
      <% } %>
    </table>
  </div>
